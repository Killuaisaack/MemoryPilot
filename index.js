// MemoryPilot v3.6.0 - SillyTavern Extension
...
//
// v3.6.0 Changes:
//   - Layered memory system (人物/场景/动态 anchors, permanent injection)
//   - New standalone anchor panel (⚓ 分层记忆)
//   - Time-awareness improvements in prompts
//   - Extract anchors from existing memory list
//   - New injection variable: {{getvar::mp_layered_ctx}}
...
async function runRecall() {
  ...
  // v3.6.0: Always inject layered memories (permanent, every turn)
        ...
        <div class="inline-drawer-content">
          <div style="display:flex;flex-direction:column;gap:8px;padding:8px 0">
            ...
            <div style="border-top:1px solid rgba(255,255,255,0.06);padding-top:8px;margin-top:4px">
              <div style="font-size:11px;color:#34d399;font-weight:500;margin-bottom:4px">⚓ 分层记忆 v3.6</div>
              <div style="font-size:10px;color:#777;line-height:1.4">
                永久锚点（人物/场景/动态）每轮注入，独立于记忆列表。<br>
                点击聊天栏「⚓ 分层记忆」按钮管理。<br>
                注入变量：<code style="color:#a78bfa;font-size:10px">{{getvar::mp_layered_ctx}}</code>
              </div>
            </div>
            <div class="mp-info" style="font-size:11px;opacity:0.6;line-height:1.5">
              v3.6.0 · 存储: extensionSettings · 零 /setvar · 不被 LWB 快照
            </div>
          </div>
        </div>
      </div>
...
function hookRecall() {
  try {
    ...
    ctx.eventSource.on(ctx.eventTypes.MESSAGE_RECEIVED, async () => {
      ...
      // Auto-summarize (unchanged from v3.5)
      try {
        const s = getSettings();
        if (s.autoSummarize) {
    ...
    ctx.eventSource.on(ctx.eventTypes.CHAT_CHANGED, async () => {
      ...
      // v3.6.0: Re-inject layered on chat change
...
function isActivated() {
  try { return localStorage.getItem(MP_ACTIVATION_KEY) === 'true'; } catch { return false; }
}

jQuery(async () => {
  console.log(`[${MODULE_NAME}] Extension loaded v3.6.0 (recall: ${getSettings().recallVersion})`);
...
function initFeatures() {
  ...
  // v3.6.0: Initial layered injection
