...
### 新功能
- **📎 事件组**：多条记忆可归为一组，当组内任一记忆被关键词召回时，同组其他成员按评分补位填充剩余槽位。在记忆列表中勾选→建组，自动生成组ID
...
- **新字段**：记忆条目新增可选字段 `事件组(groupId)`（关联事件 ID 数组），旧数据无此字段不影响使用
- **激活码**：升级后首次加载需输入激活码
...
- **事件组管理**：在列表页勾选记忆→「建组」或「加入已有组」，编辑页可查看/移除所属组
...
- **事件组召回**：已触发记忆所在组的其他成员按评分排序补位（可在过滤页开关）
