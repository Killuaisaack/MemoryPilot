...
这些内容只在与当前话题相关时自然参考，不要逐条复述，不要直接照抄原句。`;
  ...
  // Storage migration and metadata cleanup are owned by storage.js.
  // The panel must never rewrite or prune persistence state on its own.
...
楼层：{{floorRange}}`;
  ...
  if (selectedTheme !== 'light') {
    st.textContent = st.textContent.split('/* ===== MemoryPilot Day')[0];
  }
  document.head.appendChild(st);

  // ===== DOM =====
  const root=document.createElement('div');root.id=P;root.dataset.mpTheme=selectedTheme;
  root.innerHTML=`
    <div class="mask"></div>
    <div class="card">
