...
(async () => {
  ...
  const DEF_PROMPT = `分析以下对话，提取两类内容：A) 关键词召回记忆 和 B) 分层锚点+时间线。
...
格式：
{"event":"场景标题","primaryKeywords":["主召回关键词"],"secondaryKeywords":["门控关键词"],"entityKeywords":["人物名"],"entityRole":{"人物名":"major/minor/npc"},"entityAliases":{"人物名":["别名1","别名2"]},"summary":"详细摘要","timeLabel":"时间标签","timeValue":1234,"floorRange":[起始楼层号,结束楼层号],"priority":"high/medium/low"}
...
entityRole（重要！区分角色频率）：
- major：主角或核心角色（出现在大多数对话中，名字作为关键词价值低）
- minor：次要 NPC（只在特定场景出现，名字是高价值触发词）
- npc：路人/临时角色
- 例如：{"阿尔忒弥斯":"major","老布":"minor","酒馆老板":"npc"}
- 这影响召回权重：minor NPC 名被匹配时获得额外加权

entityAliases（角色别名映射）：
- 只需要写有别名/昵称/代号的角色
- 例如：{"阿尔忒弥斯":["月神","Artemis","队长"],"老布":["布鲁诺"]}
- 别名也会参与关键词匹配
...
═══ B. 分层锚点+时间线（每行一个 JSON，在 A 类之后输出）═══

从同一段对话中额外提取永久性锚点和时间线条目。这些内容将常驻注入，不依赖关键词触发。

B1. 人物锚点（有新角色出场或角色特征发生变化时输出）：
{"type":"anchor","layer":"identity","label":"人物名","content":"外貌、性格、身份、与主角关系等综合描述","dateLabel":"","tags":["标签"],"role":"major/minor/npc","aliases":["别名1","别名2"]}

B2. 场景锚点（有重要场景首次出现或物理细节值得记录时输出）：
{"type":"anchor","layer":"scene","label":"场景名称","content":"物理细节：布局、物品、光线、气味等","dateLabel":"","tags":["标签"]}

B3. 动态锚点（有关系转折、情感变化等关键时刻时输出）：
{"type":"anchor","layer":"dynamics","label":"事件名称","content":"具体发生了什么，谁对谁的态度如何变化","dateLabel":"故事内时间","tags":["标签"]}

B4. 时间线（每段对话对应一条当天/当时段的概括）：
{"type":"timeline","dateLabel":"D1/第X天/UC0087.07.10","summary":"当天要事一句话概括","importance":"normal/key/turning"}

B5. 待办事项（对话中出现的约定、承诺、伏笔、计划，需要AI持续记住直到完成）：
{"type":"todo","content":"具体约定内容（如：明天下午3点在咖啡馆见面）","dateLabel":"应该发生的时间（如：D7下午3点）"}

B 类规则：
- dateLabel 使用故事中明确出现的日期/时间，绝不写"昨天""今天"
- 如果没有明确日期，用"第X-Y层"代替
- identity 的 content 要具体（"银发及腰，琥珀色眼瞳，左手无名指戴铂金戒指"），不要概括（"外貌出众"）
- scene 的 content 要包含物理细节（"6×4米单人舱，壁挂式折叠床，左舷舷窗"），不要只写地名
- dynamics 的 content 要保留因果链，保留关键细节
- 时间线 importance：剧情转折=turning，关键事件=key，普通=normal
- 不是每段对话都需要输出所有 B 类型。没有新角色就不输出 identity，没有新场景就不输出 scene。但时间线通常每段都有。
- 待办事项只在对话中出现约定/承诺/计划/伏笔时输出。例如"明天见""下周比赛""记得带那本书"。纯粹的情绪表达（"我好开心"）不是待办。
- B 类不需要输出 primaryKeywords/secondaryKeywords，这些只属于 A 类

只输出 JSON，每行一个，不要解释。先输出所有 A 类，再输出所有 B 类。
...
楼层：{{floorRange}}`;
  ...
  $('mp_brun').onclick=async()=>{
    ...
    try{
      ...
      // v3.6: separate anchor/timeline results
      const anchorResults = [];
      const timelineResults = [];
      const todoResults = [];
      for(const o of parsed){
        try{
          if(o.type === 'anchor' && o.layer && o.label && o.content){
            anchorResults.push(o);
          } else if(o.type === 'timeline' && o.summary){
            timelineResults.push(o);
          } else if(o.type === 'todo' && o.content){
            todoResults.push(o);
          } else if(o.event&&o.summary){
            nms.push({
              ...o,
              id:gid(),
              timestamp:Date.now(),
      ...
      // v4.0: auto-save anchors, timeline, and todos
      let anchorCount = 0, tlCount = 0, todoCount = 0;
      try {
        const { addLayeredEntry, addTimelineEntry, addTodo } = await import('./layered-memory.js');
        for (const a of anchorResults) {
          addLayeredEntry(a.layer, { label: a.label, content: a.content, tags: a.tags || [], dateLabel: a.dateLabel || '', role: a.role || '', aliases: a.aliases || [] });
          anchorCount++;
        }
        for (const t of timelineResults) {
          addTimelineEntry({ dateLabel: t.dateLabel || '', summary: t.summary, importance: t.importance || 'normal' });
          tlCount++;
        }
        for (const td of todoResults) {
          addTodo({ content: td.content, dateLabel: td.dateLabel || '', source: 'auto' });
          todoCount++;
        }
        if (anchorCount || tlCount || todoCount) {
          try { const { injectLayered } = await import('./layered-memory.js'); injectLayered(); } catch {}
          toastr?.info?.('分层记忆：' + (anchorCount ? anchorCount + '锚点 ' : '') + (tlCount ? tlCount + '时间线 ' : '') + (todoCount ? todoCount + '待办 ' : ''));
        }
      } catch (layeredErr) { console.warn('[MP] 锚点自动写入失败', layeredErr); }
      if(!nms.length && !anchorCount && !tlCount && !todoCount){
      ...
      if(nms.length) {
        // 持久化结果
        await savePendingOp('batch', { status:'done', results: dedupeMemories(nms), message: nms.length + '条记忆' + (anchorCount ? ' + ' + anchorCount + '锚点' : '') + (tlCount ? ' + ' + tlCount + '时间线' : '') });
        renderBatchResults(nms);
      } else {
        $('mp_br').innerHTML='<div class="ht" style="color:#4ade80">仅提取到分层锚点（' + anchorCount + '条）和时间线（' + tlCount + '条），无关键词召回记忆</div>';
      }
    }catch(e){
      if(e.name==='AbortError'){
